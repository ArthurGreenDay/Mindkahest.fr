...
public class BlockOre extends Block
{
    ...
    public int idDropped(int par1, Random par2Random, int par3)
    {
        ...
        /*///NEW///  A jouter pour droper : Item au lieux du block
        if (blockID == Block.OreEtain.blockID)
        {
            return Item.Etain.shiftedIndex;
        }
        if (blockID == Block.OreCuivre.blockID)
        {
            return Item.Cuivre.shiftedIndex;
        }
        if (blockID == Block.OreBronze.blockID)
        {
            return Item.Bronze.shiftedIndex;
        }
        if (blockID == Block.OreMithril.blockID)
        {
            return Item.Mithril.shiftedIndex;
        }
        */
        if (blockID == Block.Orepierredesang.blockID)
        {
        	return Item.pierredesang_INGOT.shiftedIndex;
        }
        ///FIN/// /
        else
        {
